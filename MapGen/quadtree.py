...
# Inserting the point name and the point into the quadtree
for i in range(100):
    spindex.insert('p'+'{}'.format(i),(x[i],y[i],x[i]+1,y[i]+1))
...
while True:
    ...
    start_s = time.time()
    res = (spindex.intersect((current_posx-40,current_posy-40,current_posx+40,current_posy+40)))
    end_s = time.time()
    # print("Intersection generated in:",str(end_s-start_s)," seconds")
    key = res
    # print(key)
    temp_x = [pointlist[x][0] for x in key]
    temp_y = [pointlist[x][1] for x in key]
    print(current_posx,current_posy)
    temp_x.append(current_posx)
    temp_y.append(current_posy)
    # print(temp_x,temp_y)
